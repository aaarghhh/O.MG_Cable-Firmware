...
class Duckpiler():
    ...
    def retriveASCIImapping(self, asciicode):
        return str(0x00)

    def commands(self, keyboardmappping):
        commands = {
                ...
                "WAIT_FOR_NOT_PRESENT": self.cmd_waitfornotpresent,
                "IF_PRESENT":self.cmd_ifpresent,
                "IF_NOT_PRESENT": self.cmd_ifnotpresent,
                "PRO":self.cmd_pro,
                "MAN":self.cmd_man,
                "VID":self.cmd_vid,
                "PID":self.cmd_pid,
    ...
    def parseLine(self, ln, keyboardmapping):
            res = {}
            res['raw']=ln
            params = {}
    ...
    def processLine(self,ln,keyboardmappping):
        parsed_line = self.parseLine(ln,keyboardmappping)
    ...
    def readFile(self,fn,keyboardmappping):
        ...
        if os.path.exists(fn):
            with open(fn) as sf:
                ...
                for script_line in script_contents.split("\n"):
                    output.append(self.processLine(script_line, keyboardmappping))
            return output
        else:
    ...
    # TODO setting UP keybaord 
    def processKey(self,processed_line,lookup_value=None,keyboardmapping=None):
    ...
    def cmd_esc(self,processed_line):
        return self.processKey(processed_line,41)

    def cmd_pause(self,processed_line):
        return self.processKey(processed_line,72)

    def cmd_right(self,processed_line):
        return self.processKey(processed_line,79)

    def cmd_left(self,processed_line):
        return self.processKey(processed_line,80)

    def cmd_up(self,processed_line):
        return self.processKey(processed_line,82)

    def cmd_down(self,processed_line):
        return self.processKey(processed_line,81)
...
class OMGInterface():
    def __init__(self,url, datakeyboardmapping):
        self.soc = None
        self.url = None
        self.datakeyboardmapping = datakeyboardmapping
    ...
    def splitProcess(self, payload):
        ...
        if float(len(payload) / 936) > 1.0:
            ...
            # TODO FIX IF the OPS of the lenght contains less of 936char it don't need to be splitted
            this_sof = "20" + self.numToHex(1) + self.charToHex(0)
            this_eof = "30" + self.numToHex(1) + self.charToHex(0)
    ...
    def encodePayload(self,commands):
        ...
        for command in commands:
                o=compiler.processLine(self,command, self.datakeyboardmapping)
                if o is None:
    ...
    def runPayload(self,raw_script):
        ...
        split_payloads = self.split_process(payload)
...
class OMGWriter(threading.Thread):
    def __init__(self, ipaddr, slot,input_script,pretty_slot=None, keyboard_layout=None):
        super(OMGWriter, self).__init__()
        self.ip_addr = ipaddr

        #TODO DEVELOPMENT OF THE KEYBOARD ENGINE
        if (not keyboard_layout):
            self.keyboard_layout = "us"
        else:
            self.keyboard_layout = keyboard_layout

        pathjson = os.path.join(CURRENT_DIR, "resources", "%s.json" % (self.keyboard_layout))
        datajson = None
        try:
            with open(pathjson) as f:
                datajson = json.load(f)
        except:
            print("Your keyabord layout  doesn't exists!!!")
            exit(1)

        if (datajson is None):
            print("Your keyabord layout file is corrupt!!!")
            exit(1)

        url = "ws://%s/d/ws/issue"%self.ip_addr
        self.omgsock = OMGInterface(url,datajson)
        self.slot = slot
...
parser.add_argument('--keyboard', '-k',
                    help="Keyboard Layout, if you don't provide a keyboard layout the default value will be US",
                    required=False)
...
if args.outfile is not None:
    ...
    #cmd: loadslot
        ...
        if args.wifi_ssid is not None and args.wifi_password is not None and args.wifi_mode is not None:
